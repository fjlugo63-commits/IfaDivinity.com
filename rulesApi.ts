...
export async function listHouseRules(
  ...
  // Primary: Fetch from engine_rule_versions
  const { data: engineVersions } = await supabase
    .from(TABLES.engine_rule_versions)
    .select('*')
    .eq('house_id', house_id)
    .order('created_at', { ascending: false });

  if (engineVersions && engineVersions.length > 0) {
    return {
      house_id,
      active_rule_version_id: config?.active_rule_version_id || null,
      rule_versions: engineVersions,
    };
  }

  // Fallback: Fetch from legacy rule_version table
