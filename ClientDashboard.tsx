    ...
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
              <Button
                ...
                onClick={() => navigate('/client/request-consultation')}
              >
                <CalendarPlus className="h-5 w-5 text-amber-600" />
                <span className="text-xs font-medium">Request Consultation</span>
              </Button>
              <Button
                variant="outline"
        ...
        <Card className="rounded-2xl border-0 shadow-md bg-white">
          ...
          <CardContent>
            {dashboardData.recent_consultations.filter(c => c.status === 'completed').length === 0 ? (
              <div className="text-center py-6">
                <History className="h-8 w-8 text-gray-300 mx-auto mb-2" />
                <p className="text-sm text-gray-500">No completed consultations yet</p>
                <p className="text-xs text-gray-400 mt-1">Your completed readings will appear here</p>
              </div>
            ) : (
              <div className="space-y-3">
                {dashboardData.recent_consultations
                  .filter(c => c.status === 'completed')
                  .slice(0, 3)
                  .map((consultation) => (
                    <div
                      key={consultation.id}
                      className="flex items-center justify-between p-3 rounded-xl bg-green-50 hover:bg-green-100 transition-colors cursor-pointer"
                    ...
                    >
                      ...
                      <div className="text-right flex items-center gap-2">
                        <p className="text-xs text-gray-400">{formatRelativeDate(consultation.scheduled_at)}</p>
