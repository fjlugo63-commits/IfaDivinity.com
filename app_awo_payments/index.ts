...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "payment-summary" && req.method === "GET") {
      ...
      const summary = {
        total_revenue: 0, pending_amount: 0, consultation_revenue: 0, ebo_revenue: 0,
        total_transactions: payments?.length || 0, paid_count: 0, pending_count: 0, unpaid_count: 0, refunded_count: 0,
      };
    ...
    if (action === "create-consultation-payment" && req.method === "POST") {
      ...
      const { consultation_id, client_id, amount, currency, client_name } = body;
      ...
      const { data, error } = await supabaseClient
        .from("app_340b9f1944_payments")
        .insert({
          awo_id: user.id, client_id: client_id || null, consultation_id,
          payment_type: "consultation", amount: parseFloat(amount), currency: currency || "USD",
          payment_status: "unpaid", notes: client_name ? `Payment for consultation with ${client_name}` : null,
        })
        .select()
        .single();
    ...
    if (action === "create-ebo-payment" && req.method === "POST") {
      ...
      const { data, error } = await supabaseClient
        .from("app_340b9f1944_payments")
        .insert({
          awo_id: user.id, client_id: client_id || null, consultation_id: consultation_id || null,
          ebo_id, payment_type: "ebo", amount: parseFloat(amount), currency: currency || "USD",
          payment_status: "unpaid", botanica_items: botanica_items || [],
    ...
    if (action === "generate-payment-link" && req.method === "POST") {
      ...
      if (!stripeKey) {
        ...
        const { data: updated, error: updateErr } = await supabaseClient
          .from("app_340b9f1944_payments")
          .update({ stripe_payment_link_url: mockLink, payment_link_expires_at: expiresAt, payment_status: "pending", updated_at: new Date().toISOString() })
          .eq("id", payment_id)
          .select()
          .single();

        if (updateErr) throw updateErr;
        return new Response(JSON.stringify({ payment: updated, payment_link: mockLink }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }

      const amountCents = Math.round(payment.amount * 100);
      const description = payment.payment_type === "consultation" ? "Consultation Payment" : "Ebo Prescription Payment";
      ...
      if (session.error) {
        return new Response(JSON.stringify({ error: session.error.message }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }

      const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
      const { data: updated, error: updateErr } = await supabaseClient
        .from("app_340b9f1944_payments")
        .update({ stripe_checkout_session_id: session.id, stripe_payment_link_url: session.url, payment_link_expires_at: expiresAt, payment_status: "pending", updated_at: new Date().toISOString() })
        .eq("id", payment_id)
        .select()
        .single();

      if (updateErr) throw updateErr;
      return new Response(JSON.stringify({ payment: updated, payment_link: session.url }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (action === "refund" && req.method === "POST") {
      ...
      if (payment.payment_status !== "paid") {
        return new Response(JSON.stringify({ error: "Can only refund paid payments" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      ...
      if (stripeKey && payment.stripe_payment_intent_id) {
        const refundRes = await fetch("https://api.stripe.com/v1/refunds", {
          method: "POST",
          headers: { "Authorization": `Bearer ${stripeKey}`, "Content-Type": "application/x-www-form-urlencoded" },
          body: new URLSearchParams({ "payment_intent": payment.stripe_payment_intent_id, "reason": "requested_by_customer" }),
        });
        const refund = await refundRes.json();
        if (refund.error) {
          return new Response(JSON.stringify({ error: refund.error.message }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
        }
      }

      const { data, error } = await supabaseClient
        .from("app_340b9f1944_payments")
        .update({ payment_status: "refunded", refunded_at: new Date().toISOString(), refund_reason: reason || "Requested by Awo", updated_at: new Date().toISOString() })
        .eq("id", payment_id)
        .select()
        .single();
