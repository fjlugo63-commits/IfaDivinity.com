...
export default function ClientConsultations() {
  ...
  const loadConsultations = async () => {
    ...
    try {
      ...
      if (clientData) {
        // Fetch from bookings table (legacy)
        ...
        // Also fetch from consultation_requests table (new lifecycle)
        ...
        const bookingItems: Consultation[] = (bookings || []).map((b: Record<string, unknown>) => ({
          id: b.id as string,
        ...
        const requestItems: Consultation[] = (requests || []).map((r: Record<string, unknown>) => ({
          id: r.id as string,
          awo_name: (r.awo_name as string) || 'Assigned Awo',
          consultation_type: (r.service_type as string) || 'Consultation',
          scheduled_at: (r.scheduled_at as string) || (r.preferred_date as string) || (r.created_at as string),
          status: r.status as string,
          notes: r.client_notes as string | undefined,
          payment_status: r.payment_status as string | undefined,
          booking_ref: undefined,
        }));

        const combined = [...requestItems, ...bookingItems];
